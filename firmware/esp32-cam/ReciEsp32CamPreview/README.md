...
4. Deja Safari mostrando la vista (se actualiza con capturas cada 0,5 s) y
   escribe `C` + Enter en el Monitor Serial.
5. Observa simultáneamente las tres capturas y el resultado de clasificación.

La página no mantiene un stream HTTP permanente: solicita `/capture` de forma
periódica. Esto permite que Safari y el script de dataset usen el endpoint sin
bloquearse entre sí. `/capture` devuelve una sola foto JPEG y sirve para el
script de dataset.
