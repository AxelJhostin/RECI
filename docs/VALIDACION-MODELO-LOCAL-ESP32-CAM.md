...
2. MobileNetV2 local.
3. Fusión ponderada: 70 % proveedor y 30 % modelo local.

Esto produce seis predicciones observables sobre tres imágenes iguales. El
firmware conserva el voto mayoritario de las tres fusiones.

El modelo local solo conoce `plastico` y `vidrio`; no puede reconocer latas,
cartón u orgánicos. Por seguridad:

- nunca reemplaza un `desconocido` emitido por proveedor + sistema experto;
- un conflicto sin confianza suficiente termina en `desconocido`;
- si el TFLite no carga, continúa el flujo anterior de OpenAI;
- los pesos no deben cambiarse sin una matriz de confusión con imágenes
  nuevas de la ESP32-CAM.
...
Gatorade. La fusión 70/30 permite que una decisión fuerte del proveedor y el
sistema experto no sea anulada automáticamente por esos errores.

### 3.3 Capturas reales de la ESP32-CAM
...
5. Medir por separado MobileNetV2, OpenAI y la fusión sobre el mismo conjunto.
...
9. Ajustar pesos o umbral solo con evidencia y aprobación de Paula.

Meta sugerida para revisión con Paula: al menos 90 % por clase en un conjunto
independiente de la ESP32-CAM, sin aceptar latas, cartón u orgánicos como
plástico o vidrio.
